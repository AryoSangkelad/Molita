...
class EdukasiService {
  ...
  Future<List<JenisEdukasi>> getJenisEdukasi() async {
    final response = await http.get(Uri.parse('${ApiConstant.baseUrlApi}/edukasi/jenis'));
  ...
  Future<List<ArtikelEdukasi>> getArtikel() async {
    final response = await http.get(Uri.parse('${ApiConstant.baseUrlApi}/edukasi/artikel'));
    if (response.statusCode == 200) {
      List<dynamic> data = jsonDecode(response.body);
  ...
  Future<List<VideoEdukasi>> getVideo() async {
    final response = await http.get(Uri.parse('${ApiConstant.baseUrlApi}/edukasi/video'));
    if (response.statusCode == 200) {
      List<dynamic> data = jsonDecode(response.body);
